...
private:
    ...
    // Вызывается после завершения каждой асинхронной операции
    void CheckReadiness(sys::error_code ec) {
        if (delivered_) {
            // Заказ уже доставлен (в том числе с ошибкой), ничего не делаем
            return;
        }
        if (ec) {
            // При ошибке уведомляем клиента о невозможности выполнить заказ
            return Deliver(ec);
        }

        // Собираем гамбургер, когда все ингредиенты готовы
        if (CanAddOnion()) {
    ...
    [[nodiscard]] bool CanAddOnion() const {
        // Лук можно добавить, когда котлета обжарена, лук замаринован, но ещё не добавлен
        return hamburger_.IsCutletRoasted() && onion_marinaded_ && !hamburger_.HasOnion();
    }

    [[nodiscard]] bool IsReadyToPack() const {
        // Упаковываем, когда котлета обжарена и, если нужно, добавлен лук
    ...
    void Pack() {
        ...
        // Упаковка требует всего внимания работника: имитируем активное ожидание
